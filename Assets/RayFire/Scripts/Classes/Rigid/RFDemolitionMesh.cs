    ...
    public class RFDemolitionMesh
    {
        ...
        public static bool DemolishMeshV2(RayfireRigid scr)
        {
            ...
            if (scr.mshDemol.HasEngineAndMeshes == false)
            {
                ...
                RayfireMan.Log("V2 Cache Fragments " + stopWatch.Elapsed.TotalMilliseconds + " ms.");
                ...
                if (scr.mshDemol.HasEngineAndMeshes == true)
                {
                    ...
                    stopWatch2.Stop();
                    RayfireMan.Log("V2 Create Fragments " + stopWatch2.Elapsed.TotalMilliseconds + " ms.");
                    
                    stopWatch0.Stop();
                    RayfireMan.Log("V2 Total Time " + stopWatch0.Elapsed.TotalMilliseconds + " ms.");
                    RayfireMan.Log("============================================");
                    return true;
                }
            }
        ...
        public static bool SliceMeshV2(RayfireRigid scr)
        {
            ...
            if (scr.objTp == ObjectType.SkinnedMesh)
            {
                ...
                Debug.Log (rag.skins[10].skin);
                Debug.Log (rag.skins[10].boneIds.Count);
                for (int i = 0; i < rag.skins[10].boneIds.Count(); i++)
                {
                    int ind = rag.skins[10].boneIds[i];
                    Debug.Log (rag.bones[ind], rag.bones[ind].gameObject);
                }
                ...
                for (int i = 0; i < rag.chains.Count; i++)
                {
                    if (rag.chains[i].live == false)
                    {
                        ...
                        RFChain.DuplicateChain (rag.chains[i], flatParent.transform);
                        
                        // Get main parent chain root
                        Transform mainBone = rag.chains[i].MainBone();
                    }
                }
                ...
                Debug.Log ("deadBoneChains " + rag.chains.Count);
                for (int i = 0; i < rag.chains.Count; i++)
                {
                    Debug.Log (" ------- Chain " + i + " Live: " +  rag.chains[i].live);
                    for (int j = 0; j < rag.chains[i].oldBones.Count; j++)
                    {
                        Debug.Log ("Chain " + rag.chains[i].oldBones[j].name, rag.chains[i].oldBones[j].gameObject);
                        
                        /*
                        int parid = boneChains[i].parentIds[j];
                        if (parid >= 0)
                            Debug.Log ("Parent id " + boneChains[i].parentIds[j], boneChains[i].bones[parid].gameObject);
                        else
                            Debug.Log ("No Parent ");
