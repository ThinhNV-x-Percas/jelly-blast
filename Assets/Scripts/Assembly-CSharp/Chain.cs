...
public class Chain : global::UnityEngine.MonoBehaviour
{
	...
	private static float ApproxQuadLen(global::UnityEngine.Vector2 a, global::UnityEngine.Vector2 b, global::UnityEngine.Vector2 c, int n = 16)
	{
		...
		for (int i = 1; i <= n; i++)
		{
			global::UnityEngine.Vector2 point = Bezier(a, b, c, (float)i / n);
			total += global::UnityEngine.Vector2.Distance(prev, point);
			prev = point;
		}
		return total;
	}
	...
	private static float[] EvenTByArc(global::UnityEngine.Vector2 a, global::UnityEngine.Vector2 b, global::UnityEngine.Vector2 c, int linkCount, float bezLen, int table = 60)
	{
		...
		{
			global::UnityEngine.Vector2 point = Bezier(a, b, c, (float)i / table);
			cumulative[i] = cumulative[i - 1] + global::UnityEngine.Vector2.Distance(prev, point);
			prev = point;
		}
		...
		for (int i = 0; i < linkCount; i++)
		{
			...
			while (searchIndex < table - 1 && cumulative[searchIndex + 1] < targetLength)
			{
				searchIndex++;
			}
			float segmentStart = cumulative[searchIndex];
			float segmentEnd = cumulative[searchIndex + 1];
			float localT = (segmentEnd > segmentStart) ? ((targetLength - segmentStart) / (segmentEnd - segmentStart)) : 0f;
			localT = global::UnityEngine.Mathf.Clamp01(localT);
			result[i] = (searchIndex + localT) / table;
		}
		return result;
	}
