...
public class FluidCollectDisplay : FluidRendererBase
{
	...
	public override void OnPreComputeUpdate()
	{
		...
		for (int i = activeCount - 1; i >= 0; i--)
		{
			if (particles[i].mode == CollectParticleMode.Shrink)
			{
				...
				interpPositions[i] = global::UnityEngine.Vector2.Lerp(particles[i].startPos, particles[i].targetPos, shrinkT);
				scales[i] = (1f - shrinkT * shrinkT * 0.1f) * particleSize;
			}
			else if (particles[i].mode == CollectParticleMode.Explode)
			{
			...
			float glow = global::UnityEngine.Mathf.Clamp01(1f - (time - particles[i].startTime) / 0.5f);
			compute.emissionColors[computeIndex].z = 0f;
			compute.emissionColors[computeIndex].w = glow;
		}
	...
	public void RemoveParticle(int index)
	{
		if ((int)(index & 0x80000000L) == 0 && activeCount > index)
		{
			...
			if (array[index].onComplete != null)
			{
				array[index].onComplete();
			}
			...
			if (num2 != index)
			{
				...
				positions[index] = positions[num2];
				float[] array2 = scales;
