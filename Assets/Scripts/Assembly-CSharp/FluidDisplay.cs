...
public class FluidDisplay : FluidRendererBase
{
	...
	public override void OnPreComputeUpdate()
	{
		...
		if (activeCount >= 1)
		{
			...
			{
				int solverIndex = fluidSolver.idToIndex[particleIds[i]];
				...
				int computeIndex = compute.idToIndex[particleIds[i]];
				...
				compute.emissionColors[computeIndex] = new global::UnityEngine.Vector4(emission.x, emission.y, emission.z, emission.w);
			}
		}
		base.OnPreComputeUpdate();
		FluidSolver fluidSolver6 = solver;
		for (int j = 0; j < fluidSolver6.ActiveCount; j++)
		{
			global::Unity.Mathematics.float2 pos = fluidSolver6.positions[j];
			if (float.IsNaN(pos.x) || float.IsInfinity(pos.x) || float.IsNaN(pos.y) || float.IsInfinity(pos.y))
			{
				global::UnityEngine.Debug.Log("NaN fluid position detected");
			}
		}
	}
