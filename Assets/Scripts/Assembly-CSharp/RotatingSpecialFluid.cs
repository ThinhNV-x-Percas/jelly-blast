...
public class RotatingSpecialFluid : SpecialFluid
{
	...
	public override void OnPreComputeUpdate()
	{
		...
		{
			if (solver == null || !solver.idToIndex.TryGetValue(id, out var slot))
			{
				continue;
			}
			global::UnityEngine.Vector2 currentPos = solver.positions[slot];
			global::UnityEngine.Vector2 currentOffset = currentPos - position;
			global::UnityEngine.Vector2 prevOffsetValue = currentOffset;
			if (prevOffset != null && prevOffset.TryGetValue(id, out var stored))
			{
				prevOffsetValue = stored;
			}
			float distSq = currentOffset.x * currentOffset.x + currentOffset.y * currentOffset.y;
			sumCurXPrevX += currentOffset.x * distSq * prevOffsetValue.x;
			sumCurYPrevX += currentOffset.y * distSq * prevOffsetValue.x;
			sumCurXPrevY += currentOffset.x * distSq * prevOffsetValue.y;
			sumCurYPrevY += currentOffset.y * distSq * prevOffsetValue.y;
			if (prevOffset != null)
			{
				prevOffset[id] = currentOffset;
			}
		}
		float crossSum = sumCurYPrevX - sumCurXPrevY;
		float dotSum = sumCurYPrevY + sumCurXPrevX;
		float angleDelta = global::UnityEngine.Mathf.Atan2(crossSum, dotSum) * global::UnityEngine.Mathf.Rad2Deg;
		...
		_propBlock.SetVector("_Rotation", new global::UnityEngine.Vector4(zRotation, zRotation, 0f, 0f));
