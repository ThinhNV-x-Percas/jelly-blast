...
public class SpecialFluid : FluidRendererBase
{
	...
	public virtual void OnSolverRemoveParticlesStart(global::System.Collections.Generic.HashSet<int> ids, bool explode)
	{
		...
		for (int i = 0; i < activeCount; i++)
		{
			if (!ids.Contains(solverIds[i]))
			{
				if (i != newCount)
				{
					solverIds[newCount] = solverIds[i];
					computeIds[newCount] = computeIds[i];
	...
	public override void OnPreComputeUpdate()
	{
		...
		if (activeCount >= 1)
		{
			...
			for (int i = 0; i < activeCount; i++)
			{
				...
				global::Unity.Mathematics.float4 emission = fluidSolver.emissionColors[solverIndex];
				compute.emissionColors[computeIndex].y = emission.y;
				compute.emissionColors[computeIndex].z = emission.z;
				compute.emissionColors[computeIndex].w = emission.w;
			}
		}
		...
		value2.z = vector5.z;
		value2.w = 0f;
		_propBlock.SetVector("_ViewportPos", value2);
		global::UnityEngine.Vector4 value3 = default(global::UnityEngine.Vector4);
		value3.x = global::UnityEngine.Mathf.Abs(vector9.x - vector7.x);
		value3.y = global::UnityEngine.Mathf.Abs(vector9.y - vector7.y);
		value3.z = 0f;
		value3.w = 0f;
		_propBlock.SetVector("_ViewportSize", value3);
		mr.SetPropertyBlock(_propBlock);
	}
	...
	private void _003CHandlePopIn_003Eg__SetScale_007C13_0(float scale)
	{
		...
		{
			if (solver != null && solver.idToIndex.TryGetValue(particleId, out int index))
			{
	...
	private void _003CHandleFlash_003Eg__SetAlpha_007C15_0(float val)
	{
		...
		foreach (int particleId in particleIds)
		{
			if (solver != null && solver.idToIndex.TryGetValue(particleId, out int index))
			{
				global::Unity.Mathematics.float4 emission = solver.emissionColors[index];
				emission.x = val;
				solver.emissionColors[index] = emission;
			}
		}
	}
}
